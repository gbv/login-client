...
class LoginClient {
  ...
  _send(req) {
    ...
    const send = () => {
      if (!this._ws || this._ws.readyState != 1) {
        ...
        setTimeout(() => {
          send()
          currentRetryMs *= retryMult
        }, Math.min(currentRetryMs, retryMsMax))
      } else {
        try {
  ...
  _handleClose() {
    ...
    this._connected = false
    setTimeout(() => {
      this._currentRetryMs *= retryMult
      this.connect()
    }, Math.min(this._currentRetryMs, retryMsMax))
  }
